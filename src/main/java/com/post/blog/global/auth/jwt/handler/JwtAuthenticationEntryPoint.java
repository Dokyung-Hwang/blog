...
// AuthenticationEntryPoint 구현 클레스
// 인증되지 않은 사용자가 보호된 리소스에 접근하려고 할 때 Spring Security 에서 기본적으로 제공하는 오류 페이지가 아닌 커스텀 응답 반환
// 401 Unauthorized 상태 코드를 클라이언트에게 응답으로 반환하고 커스텀 에러 메세지 제공(Json)
