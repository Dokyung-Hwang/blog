...
// JWT 기반 인증 처리 과정에서 발생하는 예외를 처리하기 위한 필터(HTTP 요청마다 한 번씩 실행되도록 보장)
// 요청을 처리하는 과정에서 특정 예외가 발생했을 경우 적절한 에러 응답을 클라이언트에게 반환
@Slf4j
public class AuthExceptionFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response,
        ...
        } catch (TokenExpiredException e) {    // JWT 토큰 만료 시
        ...
        } catch (BusinessLogicException e) {    // 비즈니스 로직에서 발생하는 예외
